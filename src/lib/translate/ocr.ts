...
// 괄호/대괄호/중괄호는 순수 기호 마커 취급에서 제외한다 — "(Zn electrode)"처럼 여는 괄호 하나가
// 문장 맨 앞에 오는 경우(짝이 되는 닫는 괄호는 문장 끝에 따로 있음)까지 "여는 괄호 = 마커"로
// 잘못 인식해서 그 괄호만 떼어 보존하면, Claude가 남은 "Zn electrode)"의 안 맞는 닫는 괄호를 보고
// 스스로 여는 괄호를 새로 만들어 붙이는 바람에 "((Zn 전극)"처럼 괄호가 겹쳐 나오는 버그가 실측
// 확인됨. 괄호류는 항상 쌍으로 붙어있는 "(a)"류 패턴(아래 두 번째 대안)으로만 마커로 인정한다.
